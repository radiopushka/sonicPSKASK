...
int setup_alsa(char* recordf, char* playbackf, int buffer_size, int sampler){
	int frames=1;
  ...
  if(recordf!=NULL && playbackf!= NULL)
...
void awrite(short* input, int bsize){
  int status;
		//snd_pcm_wait(playback,-1);
	 if((status=snd_pcm_writei(playback,(char*)input,bsize ))== -EPIPE){
    snd_pcm_drain(playback);
